function HCard({ html, state: { attrs } }) {
  const {
    name,
    honorificPrefix,
    givenName,
  ...
  return html`
    <div class="h-card">
      ...
      ${honorificPrefix
        ? `<span class="p-honorific-prefix">${honorificPrefix}</span>`
        : ""}
      ${givenName ? `<span class="p-given-name">${givenName}</span>` : ""}
      ${additionalName
        ? `<span class="p-additional-name">${additionalName}</span>`
        : ""}
      ${familyName ? `<span class="p-family-name">${familyName}</span>` : ""}
      ${sortString ? `<span class="p-sort-string">${sortString}</span>` : ""}
      ${honorificSuffix
        ? `<span class="p-nahonorific-suffix">${honorificSuffix}</span>`
        : ""}
      ...
      ${postOfficeBox
        ? `<span class="p-post-office-box">${postOfficeBox}</span>`
        : ""}
      ${streetAddress
        ? `<span class="p-street-address">${streetAddress}</span>`
        : ""}
      ${extendedAddress
        ? `<span class="p-extended-address">${extendedAddress}</span>`
        : ""}
      ...
      ${genderIdentity
        ? `<span class="p-gender-identity">${genderIdentity}</span>`
        : ""}
      ${anniversary
        ? `<time class="dt-anniversary">${anniversary}</time> anniversary`
        : ""}
      <slot name="adr"></slot>
      <slot name="geo"></slot>
      <slot name="org"></slot>
