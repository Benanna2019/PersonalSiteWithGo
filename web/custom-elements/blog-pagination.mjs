function BlogPagination({ html, state }) {
  const { store } = state;
  ...
  const prevButton =
    currentIndex !== 0
      ? `<blog-pagination-button class="mie-auto mie-0-lg" index="${
          currentIndex - 1
        }" label="‹ Prev"></blog-pagination-button>`
      : "";

  const nextButton =
    currentIndex + 1 !== totalPages
      ? `<blog-pagination-button class="mis-auto mis-0-lg" index="${
          currentIndex + 1
        }" label="Next ›"></blog-pagination-button>`
      : "";
  ...
  return html`
    ...
    <nav class="block inline-block-lg text-center p0 relative">
      <ul class="flex justify-content-between">
        ...
        <div
          class="pages hidden flex-lg justify-content-center overflow-x-hidden"
        >
          ${buttons.join("")}
        </div>
